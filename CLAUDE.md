...
- Bramka akceptacji w `scripts/context-smoke.js` stoi na trzech warunkach, licząc na całej sesji: gęstość słownika `voiceDensityPerThousand >= 6` na 1000 słów, suma wpadek drugiej osoby `<= 1`, a druga połowa tur ma mieć co najmniej jedno trafienie słownika. Dwa pierwsze progi wymieniono świadomie i nie wolno ich cofnąć bez pomiaru: suma trafień `>= liczba tur` karała za zwięzłość (przy tej samej gęstości krótsza odpowiedź oblewała), a warunek proporcji między połowami sesji odrzucił 7 z 10 przebiegów z żywą personą, bo liczony z 12 liczb rzędu 0–5 wahał się 0,42–1,11 przy tej samej kotwicy. README nie powtarza tych liczb — jedynym źródłem prawdy jest kod.
