...
const BROKEN_GRAMMAR_PATTERNS = [
  ...
  /(?:^|[^\p{L}])(?:worker|indeks|węz\p{L}*|drzew\p{L}*|b-tree|circuit[- ]breaker|breaker|wyszukiwanie|regex|wzorzec|string|kod|mutacj\p{L}*|kolejk\p{L}*|producent|opóźn\p{L}*|walidacj\p{L}*|parser|build|linter|testy?|cache|baza|redis|dns|system|plugin|hook|guard|model|codex|robota)\s+(?:(?:nie|tylko|dodatkowo)\s+)?\p{L}+(?:ć|c)(?=$|[^\p{L}])/giu,
