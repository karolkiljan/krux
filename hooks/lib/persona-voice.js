...
const BROKEN_GRAMMAR_PATTERNS = [
  /(?:^|[^\p{L}])(?:cache|baza|regex|kod|testy?|kolejk\p{L}*|build|linter|dokumentacj\p{L}*|robota|system|plugin|hook|guard|model|codex|redis|dns)\s+(?:pust\p{L}*|pełn\p{L}*|paść|gnić|siedzieć|widzieć|mieć|zielon\p{L}*|czyst\p{L}*|śwież\p{L}*|trup|gotow\p{L}*|mocn\p{L}*|stabiln\p{L}*|zakończon\p{L}*|nie\s+wstać|stoi\s+mocno)(?=$|[^\p{L}])/giu,
  /(?:^|[^\p{L}])(?:worker|indeks|węz\p{L}*|drzew\p{L}*|b-tree|circuit[- ]breaker|breaker|wyszukiwanie|regex|wzorzec|string|kod|mutacj\p{L}*|kolejk\p{L}*|producent|opóźn\p{L}*|walidacj\p{L}*|parser|build|linter|testy?|cache|baza|redis|dns|api|system|plugin|hook|guard|model|codex|robota)(?:\s*:\s*(?:\d+)?)?\s+(?:(?:nie|tylko|dodatkowo)\s+)?\p{L}+(?:ć|c)(?=$|[^\p{L}])/giu,
...
function voiceSignals(text) {
  ...
  result.pass = firstPersonCount === 0
    && offerCount === 0
    && result.brokenGrammar
    && (result.compression || result.lexicon);
  return result;
}
